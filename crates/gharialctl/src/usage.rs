...
pub fn usage(long: bool) {
    ...
    println!();
    println!("Tags (1..32, per output — every screen is its own view):");
    println!("  tag focus <N>              Show only tag N on the focused screen");
    ...
    println!();
    println!("Outputs (screens):");
    println!("  output focus <TARGET>      Switch the focused screen. New windows,");
    println!("                             tag commands, and keyboard focus follow;");
    println!("                             the pointer warps along.");
    println!("  output send <TARGET>       Move the focused window to a screen");
    println!("                             <TARGET> = next|prev|left|right|up|down,");
    println!("                             a connector name (DP-1), or a 1-based index");
    println!("  output link <A:EDGE> <B:EDGE>");
    println!("                             Link two screen edges so the pointer warps");
    println!("                             through them (both directions), e.g.");
    println!("                             output link DP-1:left DP-2:right");
    println!("                             EDGE = left|right|top|bottom. Links only");
    println!("                             fire where screens are not already adjacent.");
    println!("  output unlink <A:EDGE>     Remove links touching that edge");
    println!("  output list                Describe outputs and configured links");
    println!();
