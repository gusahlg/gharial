...
//!
//! Tag view actions (`focus`/`toggle`) apply to the *focused output's*
//! active mask — every screen is an independent view into the tag
//! space. All four actions share `apply_tag_change` for the visibility
//! recompute + focus restore that follows any mutation.
...
pub(in crate::wm) fn focus_tag(world: &mut World, n: u8) {
    let Some(output) = world.outputs.focused_mut() else {
        return;
    };
    output.active_tags = tag_mask(n);
